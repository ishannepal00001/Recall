...
export function WardrobeItemCard({ item, onEdit, onDelete }: { item: WardrobeItem; onEdit?: () => void; onDelete?: () => void }) {
  ...
  return (
    <div className="group flex flex-col h-full rounded-2xl bg-white/[0.06] border border-white/10 overflow-hidden hover:border-white/20 hover:bg-white/[0.08] transition-colors">
      <div className="relative h-40 shrink-0 bg-black/20 overflow-hidden">
        ...
        <div className="absolute top-2 left-2 flex items-center gap-1.5 rounded-full bg-black/60 backdrop-blur px-2 py-1 border border-white/10">
        ...
        <span className={`absolute top-2 right-2 text-[11px] font-medium px-2 py-1 rounded-full ${statusTone[item.status] ?? 'bg-white/10 text-white'}`}>
      ...
      <div className="p-4 flex flex-col flex-1 gap-2">
        ...
        <div className="flex flex-wrap gap-1.5 mt-1 min-h-[22px]">
          {item.store_location && (
            <span className="inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-full bg-white/10 text-white/70 border border-white/10">
          ...
          {(item.status === 'to_wash' || item.status === 'needs_wash') && (
            <span className="inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-full bg-amber-500/20 text-amber-200 border border-amber-500/20">
              <Droplets size={11} /> {item.status === 'to_wash' ? 'At wash' : 'Wash needed'}
            </span>
          )}
        </div>
        <div className="flex justify-end gap-2 mt-auto pt-3">
          <button onClick={onEdit} aria-label="Edit" className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/15 border border-white/10 text-white flex items-center justify-center transition-colors">
            <Pencil size={14} />
          </button>
          <button onClick={onDelete} aria-label="Delete" className="w-8 h-8 rounded-full bg-red-500/15 hover:bg-red-500/25 border border-red-500/20 text-red-300 flex items-center justify-center transition-colors">
            <Trash2 size={14} />
          </button>
        </div>
      </div>
